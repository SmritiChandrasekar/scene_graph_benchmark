...
class COCODemo(object):
    ...
    def run_on_opencv_image(self, image, args):
        """
        Arguments:
        ...
        print(f"Actual Labels: {len(labels)}")
        labels = [self.CATEGORIES[i] for i in labels]
        print(f"Cate Labels: {len(labels)}")
        ...
        result_img = self.overlay_class_names(result, top_predictions)
        
        props = zip(labels, scores, boxes)
        ...
        image_output[op.splitext(op.split(args.image_file)[1])[0]] = result_list
    ...
    def overlay_boxes(self, image, predictions):
        ...
        print("in overlay box", len(labels))
        colorsMap = self.get_random_colors(labels)
        print("after colorMap", len(labels))
        print(colorsMap)
        colors = [colorsMap.get(label) for label in labels.tolist()]
        print("colors prop", colors, len(colors))
    ...
    def overlay_class_names(self, image, predictions):
        ...
        for box, score, label in zip(boxes, scores, labels):
            x, y = box[:2]
            print(f"X, type: {x}, {type(x.item())}")
            print(f"Y, type: {y}, {type(y.item())}")
