...
def main():
    ...
    print("--------------------------------")
    print(len(rects), len(labels))
    
    # Saving object detection data
    result_list = []
    image_output = {}
    for label, score, rect in zip(labels, scores, rects):
        result = {}
        result["label"] = label
        result["bounding_box"] = rect
        result["score"] = score
        result_list.append(result)
    image_output[op.splitext(op.split(args.img_file)[1])[0]] = result_list
    key = op.splitext(op.split(args.img_file)[1])[0]
    text_save_file = 'detection_output.json'
    write_json(image_output, text_save_file)
        

    # save results in text
    print(cfg.MODEL.ATTRIBUTE_ON)
    print(args.visualize_attr)
    if cfg.MODEL.ATTRIBUTE_ON and args.visualize_attr:
        result_str = ""
        for label, score, attr_score, rect in zip(labels, scores, attr_scores, rects):
            print(rect)
            result_str += "Label prediction\n"
            result_str += label+'\n'
            result_str += "Rectangle Bounding Box\n"
            result_str += ','.join([str(_data) for _data in rect]) + '\n'
            result_str += "Score attributes\n"
